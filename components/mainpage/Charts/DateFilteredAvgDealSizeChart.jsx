...
export default function DateFilteredAvgDealSizeChart({ lineColor, granularity, filled = false }) {
  ...
  const [series, setSeries] = useState([]); // [{ unit, label, avgDealSize }]
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError('');
      try {
        const res = await fetch(`/api/analytics/newpagecharts/avgDealSize?${params}`);
        const json = await res.json();
        if (!json.success) throw new Error(json.message || 'Failed to load avg deal size');
        setSeries(json.data || []);
        setUnitUsed(json.unit || '');
      } catch (e) {
        console.error('[AvgDealSizeChart]', e);
        setError(e?.message || 'Failed to load avg deal size');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const values = useMemo(() => series.map(d => d.avgDealSize ?? 0), [series]);
  ...
  const options = useMemo(() => ({
    ...
    plugins: {
      ...
      tooltip: {
        ...
        borderWidth: 1,
        callbacks: {
          label: (ctx) => `Avg Deal: $${Number(ctx.parsed.y || 0).toLocaleString()}`
        }
      },
    },
    scales: {
      ...
      y: {
        ...
        grid: { color: alpha(theme.palette.divider, 0.2) },
        title: { display: true, text: 'Avg Deal Size', color: theme.palette.text.secondary },
        beginAtZero: true,
      },
    },
  }), [theme]);
