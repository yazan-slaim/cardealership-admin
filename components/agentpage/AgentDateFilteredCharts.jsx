 ...
 */
export default function AgentRevenueLineChart({ agentId, lineColor, granularity, filled = false }) {
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ agentId, startDate, endDate });
    ...
    (async () => {
      ...
      setError('');
      try {
        const res = await fetch(`/api/employee/fetch-chart-filter?${params}`);
  ...
  }, [agentId, startDate, endDate, granularity]);
